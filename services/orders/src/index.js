...
app.get('/orders', ah(async (req, res) => {
  ...
  const { rows } = await pool.query(
    'SELECT id, status, total_cents, created_at FROM orders WHERE user_id = $1 ORDER BY created_at DESC, id DESC LIMIT $2 OFFSET $3',
    [userId, limit, offset],
  );
...
app.get('/orders/:id', ah(async (req, res) => {
  ...
  const { rows } = await pool.query(
    'SELECT id, status, total_cents, shipping_address, created_at FROM orders WHERE id = $1 AND user_id = $2', [id, userId]);
