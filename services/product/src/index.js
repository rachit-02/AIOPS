...
app.get('/products', ah(async (req, res) => {
  const { rows } = await pool.query('SELECT id, sku, name, price_cents, stock FROM products ORDER BY id');
  res.json(rows);
}));
...
app.get('/products/:id', ah(async (req, res) => {
  const { rows } = await pool.query('SELECT id, sku, name, price_cents, stock FROM products WHERE id = $1', [
